...
from schemas.user import Client, ClientRegister, Master, MasterRegister, ClientEdit, MasterEdit, UserEdit, \
    MasterIn, MasterWithName, RefreshToken, UnreadMessageOut, PaymentOut, RecoverPasswordIn, ChangePasswordIn
from utils.service_result import handle_result
from utils.dependencies import get_current_user, client_checker, user_checker, is_user_active
...
async def verify_email(code: str, user=Depends(get_current_user), db: get_db = Depends()):
    result = ClientService(db).verify_email(user, code)
    return handle_result(result)


@router.post('/send-phone-code')
async def send_phone_code(bg_tasks: BackgroundTasks, user=Depends(get_current_user), db: get_db = Depends()):
    result = await ClientService(db).send_phone_code(user, bg_tasks)
    return handle_result(result)


@router.post('/verify-phone/{code}')
async def verify_phone(code: str, user=Depends(get_current_user), db: get_db = Depends()):
    result = ClientService(db).verify_phone(user, code)
    return handle_result(result)


@router.post("/login/")
async def auth_user(user: OAuth2PasswordRequestForm = Depends(), db: get_db = Depends()):
...
@router.get('/me', summary='Get details of currently logged in user', response_model=Client)
...
async def add_master(master: MasterIn, db: get_db = Depends(), user=Depends(get_current_user)):
...
async def patch_client(
    ...
    result = ClientService(db).patch_client(user.id, data.model_dump(exclude_unset=True, exclude_defaults=True), file)
    return handle_result(result)
...
async def patch_master(username: str, data: MasterEdit, user=Depends(get_current_user), db: get_db = Depends()):
    result = MasterService(db).patch_master(username, data.model_dump(exclude_unset=True), user)
    return handle_result(result)
...
async def patch_user(
    ...
    result = UserService(db).patch_user(id, data.model_dump(exclude_unset=True, exclude_defaults=True),
                                        file, pictures, user)
    return handle_result(result)
...
@router.post('/replenish-balance/{amount}')
async def update_balance(amount: float, user=Depends(get_current_user), is_active=Depends(is_user_active),
                         db: get_db = Depends()):
    result = UserService(db).update_balance(user, amount)
    return handle_result(result)


@router.get('/confirm-payment/{payment_id}')
...
@router.get('/deposit-history', response_model=List[PaymentOut])
...
@router.post('/verify-password-recovery/{code}')
