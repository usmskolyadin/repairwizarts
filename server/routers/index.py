...
from schemas.index import Article, Review, ReviewIn, CoverPicture, City, Counters, ArticleComment, ArticleCommentIn
from services.index import ArticleService, ReviewService, CoverPictureService, CityService, CounterService
...
@router.patch('/like-article/{id}', response_model=str)
...
@router.patch('/dislike-article/{id}', response_model=str)
async def dislike_article(id: int, user=Depends(get_current_user), db: get_db = Depends()):
...
async def create_article_comment(id: int, data: ArticleCommentIn, user=Depends(get_current_user), db: get_db = Depends()):
...
async def dislike_comment(id: int, user=Depends(get_current_user), db: get_db = Depends()):
