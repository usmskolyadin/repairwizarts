...
from services.submission import OrderService, RequestService, OfferService, FeedbackService
from schemas.submission import Order, OrderIn, OrderEdit, RequestIn, Request, RequestEdit, Offer, OfferIn, OfferEdit, \
    Feedback, FeedbackIn, FeedbackEdit
from utils.service_result import handle_result
from config.database import get_db
from typing import List, Union
from utils.dependencies import get_current_user, request_checker, request_edit_checker, is_user_active, feedback_checker
...
async def create_order(data: OrderIn, bg_tasks: BackgroundTasks, user=Depends(get_current_user),
                       is_active=Depends(is_user_active), db: get_db = Depends()):
...
@router.get('/client-orders/', response_model=List[Order])
...
@router.get('/master-orders/', response_model=List[Order])
...
async def patch_order(id: int, data: OrderEdit, user=Depends(get_current_user), is_active=Depends(is_user_active),
                      db: get_db = Depends()):
    result = OrderService(db).patch_order(id, data, user)
    return handle_result(result)


@router.patch('/finish-order/{id}', response_model=dict)
async def finish_order(id: int, status: StatusEnum, user=Depends(get_current_user),
                       is_active=Depends(is_user_active), db: get_db = Depends()):
...
async def delete_order(id: int, user=Depends(get_current_user), is_active=Depends(is_user_active),
                       db: get_db = Depends()):
...
async def create_request(bg_tasks: BackgroundTasks, data: RequestIn = Depends(request_checker), files: List[UploadFile] = None,
                         user=Depends(get_current_user), is_active=Depends(is_user_active), db: get_db = Depends()):
...
@router.get('/master-requests', response_model=List[Request])
async def get_requests_by_master(user=Depends(get_current_user), db: get_db = Depends()):
    result = RequestService(db).get_requests_by_master(user)
    return handle_result(result)


@router.get('/client-requests', response_model=List[Request])
async def get_requests_by_client(user=Depends(get_current_user), db: get_db = Depends()):
    result = RequestService(db).get_requests_by_client_id(user.id)
    return handle_result(result)


@router.patch('/request/{id}', response_model=Request)
async def patch_request(id: int, data: RequestEdit, user=Depends(get_current_user), is_active=Depends(is_user_active),
                        db: get_db = Depends()):
    result = RequestService(db).patch_request(id, data, user.id)
    return handle_result(result)


@router.patch('/complete-request/{id}', response_model=dict)
async def complete_request(id: int, status: StatusEnum, user=Depends(get_current_user),
                           is_active=Depends(is_user_active), db: get_db = Depends()):
...
async def delete_request(id: int, user=Depends(get_current_user), is_active=Depends(is_user_active),
                         db: get_db = Depends()):
...
async def get_offer(request_id: int, user=Depends(get_current_user), db: get_db = Depends()):
    result = OfferService(db).get_offer(request_id, user)
    return handle_result(result)


@router.get('/offers', response_model=List[Offer])
async def get_offers_by_submission(request_id: int, user=Depends(get_current_user), db: get_db = Depends()):
    result = OfferService(db).get_offers_by_submission(request_id)
    return handle_result(result)


@router.get('/master-offers', response_model=List[Offer])
...
async def patch_offer(id: int, data: OfferEdit, user=Depends(get_current_user),
                      is_active=Depends(is_user_active), db: get_db = Depends()):
    result = OfferService(db).patch_offer(id, data, user)
    return handle_result(result)


@router.patch('/accept-offer/{id}', response_model=dict)
async def accept_offer(id: int, user=Depends(get_current_user), is_active=Depends(is_user_active),
                       db: get_db = Depends()):
...
async def delete_offer(id: int, user=Depends(get_current_user), is_active=Depends(is_user_active),
                       db: get_db = Depends()):
...
async def create_feedback(data: FeedbackIn = Depends(feedback_checker), pictures: List[UploadFile] = None,
                          user=Depends(get_current_user), is_active=Depends(is_user_active),
                          db: get_db = Depends()):
...
async def patch_feedback(id: int, data: FeedbackEdit, user=Depends(get_current_user), is_active=Depends(is_user_active),
                         db: get_db = Depends()):
...
async def delete_feedback(id: int, user=Depends(get_current_user), is_active=Depends(is_user_active),
                          db: get_db = Depends()):
