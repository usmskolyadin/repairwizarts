...
from services.service import ServiceTypeService, DeviceService, CategoryService, RepairTypeService
from schemas.service import ServiceType, Device, Category, RepairType, MasterRepair, MasterRepairEdit, AllServices, \
    RepairTypeIn, RepairTypeEdit
...
async def create_repair_type(data: RepairTypeIn, user=Depends(get_current_user), db: get_db = Depends()):
...
async def patch_repair_type(id: int, data: RepairTypeEdit, user=Depends(get_current_user), db: get_db = Depends()):
...
async def patch_master_repair(repair_id: int, data: MasterRepairEdit, user=Depends(get_current_user),
                              db: get_db = Depends()):
...
async def delete_master_repair(repair_id: int, user=Depends(get_current_user), db: get_db = Depends()):
...
async def get_master_services(master_username: str, user=Depends(get_current_user), db: get_db = Depends()):
    result = RepairTypeService(db).get_master_services(master_username)
    return handle_result(result)


@router.get('/all-services', response_model=AllServices)
