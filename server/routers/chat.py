...
async def create_dialog(data: DialogIn, user=Depends(get_current_user), is_active=Depends(is_user_active),
                        db: get_db = Depends()):
...
async def get_messages(dialog_id: int, user=Depends(get_current_user), db: get_db = Depends()):
