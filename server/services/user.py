...
from schemas.user import ClientRegister, MasterRegister, RefreshToken, MasterIn, RecoverPasswordIn, ChangePasswordIn
...
from fastapi import UploadFile, WebSocket, WebSocketDisconnect, BackgroundTasks, HTTPException
...
class UserService(AppService):
    def auth_user(self, user: OAuth2PasswordRequestForm) -> ServiceResult:
        ...
        if not tokens:
            return ServiceResult(AppException.NotFoundException(detail='Пользователь не был найден!'))
    ...
    def patch_user(self, id: int, data: dict, file: UploadFile, pictures: List[UploadFile],
                   user: models.user.Client) -> ServiceResult:
        if id != user.id:
            return ServiceResult(AppException.ForbiddenException(detail='Нет доступа!'))
        if file:
            if file.size > 10 * 1024 * 1024:
                return ServiceResult(AppException.TooLargeException('Изображение превышает размер в 10МБ'))
        try:
...
class ClientService(UserService):
    async def create_client(self, client: ClientRegister) -> ServiceResult:
        ...
        if not dbclient:
            return ServiceResult(AppException.RegistrationException(detail='Ошибка регистрации!'))
    ...
    async def send_email_code(self, user: models.user.Client) -> ServiceResult:
        ...
        if not response:
            return ServiceResult(AppException.NotFoundException('Ошибка подтверждения почты!'))
        return ServiceResult(response)
    ...
    async def send_phone_code(self, user: models.user.Client, bg_tasks: BackgroundTasks) -> ServiceResult:
    ...
    def get_client(self, id: int) -> ServiceResult:
        ...
        if not client:
            return ServiceResult(AppException.NotFoundException(detail='Пользователь не найден!'))
...
class MasterService(UserService):
    async def create_master(self, id: int | None, master: MasterIn | MasterRegister) -> ServiceResult:
    ...
    def patch_master(self, username: str, data: dict, user: models.user.Client) -> ServiceResult:
        if username != user.master[0].username:
            return ServiceResult(AppException.ForbiddenException(detail='Доступ запрещен!'))
