...
from schemas.service import MasterRepairEdit, RepairTypeIn, RepairTypeEdit, MasterRepair, AllServices
...
class DeviceService(AppService):
    def get_device(self, id: int) -> ServiceResult:
        ...
        if not device:
            return ServiceResult(AppException.NotFoundException(detail='Устройство не найдено!'))
...
class CategoryService(AppService):
    def get_category(self, id: int) -> ServiceResult:
        ...
        if not category:
            return ServiceResult(AppException.NotFoundException(detail='Категория не найдена!'))
...
class RepairTypeService(AppService):
    def create_repair_type(self, data: RepairTypeIn, user: models.user.Client) -> ServiceResult:
    ...
    def get_repair_type(self, id: int) -> ServiceResult:
        ...
        if not repair_type:
            return ServiceResult(AppException.NotFoundException(detail='Вид ремонта не найден!'))
    ...
    def patch_repair_type(self, id: int, data: RepairTypeEdit, user: models.user.Client) -> ServiceResult:
        master = UserCRUD(self.db).get_master_by_client(user)
        new_repair_type = ServiceCRUD(self.db).update_repair_type(id, data, master)
        return ServiceResult(new_repair_type)

    def patch_master_repair(self, repair_id: int, data: MasterRepairEdit, user: models.user.Client) -> ServiceResult:
        master = UserCRUD(self.db).get_master_by_client(user)
        master_repair = ServiceCRUD(self.db).update_master_repair(repair_id, data, master)
        ...
        for master_repair in master_repairs:
            ...
            if master_username:
                if repair_type.is_custom and repair_type.created_by != master_username:
                    continue
            if master_repair.master_id != master_username and master_username is not None:
                continue
            device = handle_result(DeviceService(self.db).get_device(repair_type.device_id))
    ...
    def delete_master_repair(self, repair_id: int, user: models.user.Client) -> ServiceResult:
        master = UserCRUD(self.db).get_master_by_client(user)
        response = ServiceCRUD(self.db).delete_master_repair_by_repair_id(repair_id, master)
